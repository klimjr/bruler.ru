<div class="relative">
    <div
        class="w-full rounded-2xl border border-grey-200 bg-white h-12 px-4 py-1 text-sm relative focus-within:border-[1px] focus-within:border-black {{ $errors->has($name) ? 'error-field border-red' : '' }}">
        <input
            id="{{ $id }}"
            type="{{ $type }}"
    ...
    @if (!$errors->has($name))
        <span class="absolute left-4 top-[100%] text-color-111 text-[11px]">{{ $caption }}</span>
    @endif

    @error($name)
    <span class="absolute left-4 top-[100%] text-red text-[11px]">{{ $message }}</span>
    @enderror
</div>
